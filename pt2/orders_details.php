...
<body>
	<center>
		...
		try {
			...
			$stmt = $conn->prepare("SELECT * FROM tbl_orders_a173823, tbl_staffs_a173823,
				tbl_customers_a173823 WHERE
				tbl_orders_a173823.fld_staff_num = tbl_staffs_a173823.fld_staff_num AND
				tbl_orders_a173823.fld_customer_num = tbl_customers_a173823.fld_customer_num AND
				fld_order_num = :oid");
		...
		<form action="orders_details.php" method="post">
			...
			<select name="pid">
				...
				try {
					$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
					$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					$stmt = $conn->prepare("SELECT * FROM tbl_products_a173823");
					$stmt->execute();
		...
		<table border="1">
			...
			try {
				...
				$stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a173823,
					tbl_products_a173823 WHERE
					tbl_orders_details_a173823.fld_product_num = tbl_products_a173823.fld_product_num AND
					fld_order_num = :oid");
				$stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
				$oid = $_GET['oid'];
				$stmt->execute();
