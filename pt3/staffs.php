  		...
  		<div class="row">
  			<div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
  				...
  				<form action="staffs.php" method="post" class="form-horizontal">
  					...
  					<!-- Gender -->
  					<div class="form-group">
  						<label for="staffg" class="col-sm-3 control-label">Gender</label>
  						<div class="col-sm-9">
  							<div class="radio">
  								<label>
  									<input name="gender" type="radio" id="staffg" value="Male" <?php if(isset($_GET['edit'])) if($editrow['fld_staff_gender']=="Male") echo "checked"; ?> required> Male
  								</label>
  							</div>
  							<div class="radio">
  								<label>
  									<input name="gender" type="radio" id="staffg" value="Female" <?php if(isset($_GET['edit'])) if($editrow['fld_staff_gender']=="Female") echo "checked"; ?>> Female
  								</label>
  							</div>
  						</div>
  					</div>
  					...
  					<!-- Email Address -->
  					<div class="form-group">
  						<label for="emailadd" class="col-sm-3 control-label">Last Name</label>
  						<div class="col-sm-9">
  							<input name="email" type="text" class="form-control" id="emailadd" placeholder="Email Address" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_staff_email']; ?>" required>
  						</div>
  					</div>
  				...
  				<table class="table table-striped table-bordered">
     				...
     				// Read
  					...
  					try {
  						...
  						$stmt = $conn->prepare("select * from tbl_staffs_a173823 LIMIT $start_from, $per_page");
  						$stmt->execute();
  				...
  				<nav>
  					<ul class="pagination">
  						...
  						try {
  							$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
  							$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  							$stmt = $conn->prepare("SELECT * FROM tbl_staffs_a173823");
  							$stmt->execute();
  							$result = $stmt->fetchAll();
